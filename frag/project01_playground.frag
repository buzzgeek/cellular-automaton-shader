...
vec3 calculate(vec2 st, vec3 totCol)
{
	...
	gain = 0.8;
	birth_rate = vec3(0.0, 0.0, 0.0001);
	death_rate = vec3(0.0, 0.0, 0.0);

	consumption_rate = vec3(0.01, 0.01, 0.0);
	starvation_rate = vec3(0.99, 0.7, 0.0);
	minimum_resource = vec3(0.9, 0.9, 0.8);

	competition_rate = vec3(0.0, 0.0, 0.0); // seems very necessary
	influence_rate = vec3(0.0,0.0,0.0); // eg polution would be negative
	effective_population = vec3(0.5, 0.5, 0.9);
...
//	d = normalize(d);
	...
	float rn = smoothstep(0.0, r, distance(st, vec2(0.5))); 
	float gn = smoothstep(0.0, g, distance(st, vec2(0.5))); 
	float bn = smoothstep(0.0, b, distance(st, vec2(0.5))); 
