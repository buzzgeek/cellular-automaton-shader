...
float diffU = 0.25;//0.25;
float diffV = 0.01;//0.05;
float f = 0.1;//0.1;
float k = 0.063;//0.063;
...
vec3 effictive_population = vec3(1.0);// the minimum size to be considered a population
...
vec3 calculate(vec2 st, vec3 totCol)
{
	...
	gain = 0.9;
	birth_rate = vec3(0.1, 0.1, 0.0001);
	death_rate = vec3(0.1, 0.001, 0.0);

	consumption_rate = vec3(0.2, 0.001, 0.0);
	starvation_rate = vec3(0.8, 0.1, 0.0);
	minimum_resource = vec3(0.9, 0.5, 1.0);

	competition_rate = vec3(0.0, 0.0, 0.0); // seems very necessary
	influence_rate = vec3(-0.5,0.1,0.0); // eg polution would be negative
	effictive_population = vec3(0.9, 0.1, 0.9);
	...
	if( competition_rate.r != 0.0 )
		if(r<b || r<g)
			(d.r+=competition_rate.r);
		else
			(d.r-=competition_rate.r);
	if( competition_rate.g != 0.0 )
		if(g<b || g<r)
	...
	if(consumption_rate.r!=0.0)
	{
		...
		{
			(d.r+=consumption_rate.r);
			(d.g-=consumption_rate.r*0.5); // eat 
			(d.b-=consumption_rate.r*0.5); // eat
		}
		else
			(d.r-=starvation_rate.r); // starve
	}
	if(consumption_rate.g!=0.0)
	{
		if(b > minimum_resource.b || r > minimum_resource.r)
		{
			(d.g+=consumption_rate.g);
			(d.r-=consumption_rate.g*0.5); // eat 
			(d.b-=consumption_rate.g*0.5); // eat
		}
		else
			(d.g-=starvation_rate.g); // starve
	}
	if(consumption_rate.b!=0.0)
	{
		if(g > minimum_resource.g || r > minimum_resource.r)
		{
			(d.b+=consumption_rate.b);
			(d.r-=consumption_rate.b*0.5); // eat 
			(d.g-=consumption_rate.b*0.5); // eat
		}
		else
			(d.b-=starvation_rate.b); // starve
	}
	...
	if(influence_rate.r!=0.0)
	{
		if(r>effictive_population.r)
		{
			(d.g+=influence_rate.r);
			(d.b+=influence_rate.r);
		}
	}

	if(influence_rate.g!=0.0)
	{
		if(g>effictive_population.g)
		{
			(d.r+=influence_rate.g);
			(d.b+=influence_rate.g);
		}
	}

	if(influence_rate.b!=0.0)
	{
		if(b>effictive_population.b)
		{
			(d.r+=influence_rate.b);
			(d.g+=influence_rate.b);
		}
	}
	//d = normalize(d);
...
//	}
	...
	r -= population_filter.r;
	g -= population_filter.g;
	b -= population_filter.b;
...
vec4 referenceFnc(vec2 st, vec2 pixel)
{
...
void main() {
	...
	if(isInitialized(st, pixel))
	{
		if(distance(st, pixel) > reserved_range)
			...
			//gl_FragColor = referenceFnc(st, pixel);
