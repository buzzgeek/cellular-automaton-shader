...
vec3 calculate(vec2 st, vec3 totCol)
{
	...
	gain = 0.9;
	birth_rate = vec3(0.004, 0.006, 0.002);
	death_rate = vec3(0.06, 0.12, 0.04);

	consumption_rate = vec3(0.003, 0.005, 0.003);
	starvation_rate = vec3(0.004, 0.006, 0.005);
	minimum_resource = vec3(0.1, 0.2, 0.03);

	competition_rate = vec3(0.01, 0.03, 0.05); // seems very necessary
	influence_rate = vec3(0.0,0.0,0.0); // eg polution would be negative
	effective_population = vec3(0.01, 0.08, 0.02);
