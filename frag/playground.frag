...
{ 
	...
	gain = 0.7;
	birth_rate = vec3(0.1, 0.1, 0.1);
	death_rate = vec3(0.1, 0.1, 0.1);

	consumption_rate = vec3(0.1, 0.1, 0.1);
	starvation_rate = vec3(0.1, 0.1, 0.1);
	minimum_resource = vec3(0.2, 0.2, 0.1);

	competition_rate = vec3(0.0, 0.0, 0.0); // seems very necessary
	influence_rate = vec3(-0.1,0.1,-0.1); // eg polution would be negative
	effective_population = vec3(0.1, 0.1, 0.1);
	
	population_filter = vec3(0.1, 0.1, 0.1); // removes brightness/bloom
	
	mutation = vec3(0.0, 10.0, 0.0);
	...
	{
		if(d.b > minimum_resource.b || d.g > minimum_resource.g)
		{
	...
	// mutation is rare and not very effective - eg. disabled for now
//	if(useMutation)
//	{
//		switch( int(mod(random(u_time), 100.0)) ){
//			case 0:
//				if(mutation.r!=0.0)
//					d.r += (mod(random(u_time), RAND_MAX) * mutation.r);
//				break;
//			case 1:
//				if(mutation.g!=0.0)
//					d.g += (mod(random(u_time), RAND_MAX) * mutation.g);
//				break;
//			case 2:
//				if(mutation.b!=0.0)
//					d.b += (mod(random(u_time), RAND_MAX) * mutation.b);
//				break;
//		}
//	}
	//d = lerp(d);
...
//	d = lerp(d);
	...
	if(!isFlat)
	{
		...
		rn = smoothstep(rn, r, distance(st, vec2(0.5))); 
		gn = smoothstep(gn, g, distance(st, vec2(0.5))); 
		bn = smoothstep(bn, b, distance(st, vec2(0.5))); 
			
		r = mix(r, rn, distance(st, vec2(0.5)));
		g = mix(g, gn, distance(st, vec2(0.5)));
		b = mix(b, bn, distance(st, vec2(0.5)));
