...
vec3 effective_population = vec3(11.0);// the minimum size to be considered a population
...
vec3 calculate(vec2 st, vec3 totCol)
{
	...
	gain = 0.9;
	birth_rate = vec3(0.2, 0.3, 0.1);
	death_rate = vec3(0.2, 0.1, 0.1);

	consumption_rate = vec3(0.4, 0.5, 0.3);
	starvation_rate = vec3(0.2, 0.4, 0.5);
	minimum_resource = vec3(0.1, 0.2, 0.3);

	competition_rate = vec3(0.5, 0.3, 0.5); // seems very necessary
	influence_rate = vec3(0.0,0.0,0.0); // eg polution would be negative
	effective_population = vec3(0.1, 0.8, 0.2);
	
	population_filter = vec3(0.0, 0.0, 0.0); // removes brightness/bloom
